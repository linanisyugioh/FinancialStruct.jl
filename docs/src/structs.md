...
1. 解析 [deps/include/](../../deps/include/) 下的 C 头文件（`md_def.h`、`trade_def.h`、`om_data_types.h`、`tb_lib_def.h`）；
2. 调用 `include("struct/*.jl")` 加载所有 HDB 二进制存储结构定义；
3. 为每个结构体创建对应的 Julia bitstype 别名（前缀 `c`，如 `cOrder`、`cTrade`、`cSecurityTickData`）；
