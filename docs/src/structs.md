...
上期所 CTP 接口共定义 **386** 个 `CThostFtdc*Field` 结构体（用户登录、报单、查询、风控、结算、银期转账、行情、组合等全套）。模块**已为全部 386 个结构体自动生成 `cCtp*` 别名**——与上游头文件 1:1 对齐，无需手工增删，详见 [src/FinancialStruct.jl](../../src/FinancialStruct.jl)。

```julia
using FinancialStruct
# 用法一致：与已有 cOrder/cTrade 一样可直接构造、传给 ccall、unsafe_load 等
sizeof(FinancialStruct.cCtpInputOrder)          # 直接得到该 CTP 结构体的字节大小
sizeof(FinancialStruct.cCtpQryDepthMarketData)
```
...
# ...
req = FinancialStruct.cCtpReqUserLogin(
...
下面只挑选量化交易最常用的代表性子集快速速查；**完整 386 个别名**请直接看 [src/FinancialStruct.jl](../../src/FinancialStruct.jl) 的 CTP 段，或在 Julia REPL 里 `filter(s -> startswith(string(s), "cCtp"), names(FinancialStruct, all=true))` 一次列出。

**会话/认证**
- `cCtpRspInfo` — 通用应答信息（错误码 + 错误消息）
- `cCtpReqUserLogin` / `cCtpRspUserLogin` — 用户登录请求/应答
- `cCtpUserLogout` / `cCtpForceUserLogout` — 用户登出 / 强制登出
- `cCtpReqAuthenticate` / `cCtpRspAuthenticate` — 客户端认证
- `cCtpUserPasswordUpdate` / `cCtpTradingAccountPasswordUpdate` — 口令变更
- `cCtpSettlementInfo` / `cCtpSettlementInfoConfirm` — 结算单 / 结算单确认

**委托/撤单**
- `cCtpInputOrder` / `cCtpInputOrderAction` — 报单录入 / 撤单录入
- `cCtpOrder` / `cCtpOrderAction` — 报单回报 / 撤单回报
- `cCtpTrade` — 成交回报
- `cCtpExchangeOrder` / `cCtpExchangeTrade` — 交易所原始报单 / 成交
- `cCtpRemoveParkedOrder` — 删除预埋单

**账户/持仓**
- `cCtpInvestor` — 投资者基本信息
- `cCtpInvestorPosition` — 投资者持仓汇总
- `cCtpInvestorPositionDetail` / `cCtpInvestorPositionCombineDetail` — 持仓明细 / 组合持仓明细
- `cCtpTradingAccount` — 资金账户

**合约/行情**
- `cCtpInstrument` / `cCtpCombInstrumentGuard` — 合约 / 组合合约保护
- `cCtpDepthMarketData` — 深度行情
- `cCtpSpecificInstrument` — 订阅合约回报
- `cCtpForQuoteRsp` / `cCtpQuote` — 询价应答 / 报价
- `cCtpInstrumentMarginRate` / `cCtpInstrumentCommissionRate` — 合约保证金率 / 手续费率
...
- `cCtpQrySettlementInfo` / `cCtpQrySettlementInfoConfirm`
- `cCtpQryInstrumentMarginRate` / `cCtpQryInstrumentCommissionRate`
- `cCtpQryExchange` / `cCtpQryProduct` / `cCtpQryTradingCode`
- `cCtpQryNotice` / `cCtpQryBrokerTradingParams`

**银期转账（Transfer）**
- `cCtpReqTransfer` / `cCtpRspTransfer` — 银行/期货转账请求/应答
- `cCtpReqRepeal` / `cCtpRspRepeal` — 转账冲正
- `cCtpReqQueryAccount` / `cCtpRspQueryAccount` — 银行账户查询
...
5. CTP 结构体（`cCtp*` 前缀）原始定义在 [ThostFtdcUserApiStruct.h](../../deps/include/ThostFtdcUserApiStruct.h)，使用上期所标准 typedef（`TThostFtdc*Type`）作为字段类型，CBinding 会沿 `#include` 链自动解析；不需要在 Julia 端额外做 typedef 还原。模块已为**全部 386 个** CTP 结构体自动生成 `cCtp*` 别名，与上游头文件 1:1 对齐。
